...
export function TemplatePromptForm({
  onSuccess,
  onBack,
  isSubmitting,
  setIsSubmitting,
  ...
  const getFormSchema = () => {
    ...
    const variableFields = selectedTemplate.variables.reduce(
      (acc, varName) => {
        ...
        {templates.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-muted-foreground">
              No templates found. Create some prompts with variables first.
            </p>
  ...
  const handleSubmit = async (data: any) => {
    ...
    const { name, ...variables } = data as BaseFormData &
                ...
                <div className="flex flex-wrap gap-2 mt-4 mb-2 border-b pb-2 w-full max-w-[500px]">
                  {selectedTemplate.variables.map((variable) => {
                    ...
                    return (
                      <div
                        key={variable}
                        className={`text-xs px-2 py-1 rounded max-w-[100px] truncate ${value
